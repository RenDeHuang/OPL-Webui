...
## 2026-06-19 - governance-test-compaction
- summary: Compacted the phase-specific `tests/health/product-debt-retirement.test.mjs` into durable health gates. MVP/cloud transition naming checks now live in `tests/health/stale-retirement-guard.test.mjs`; the 1000-line hard-cap source assertions now live in `tests/health/repo-bloat.test.mjs`; the registry entry was removed. This frees one durable file and one test-file budget slot without changing product behavior.
- verified: RED `node --test tests/contract/change-package-lifecycle.test.mjs` failed first because `tests/health/product-debt-retirement.test.mjs` still existed; GREEN targeted lifecycle/registry/bloat/stale tests passed; active-state `git diff --check`, `npm run verify`, `npm run gate:review`, `npm run repo:bloat`, `sentrux check .`, and `sentrux gate .` passed.
- cannot claim: product behavior change, deploy behavior change, runtime/billing/storage behavior, OPL execution/mutation, or production rollout evidence.

## 2026-06-19 - production-dogfood-evidence-foldback
- summary: Folded Cloud Rollout run `27823251419` into fixed release truth. The run executed commit `73cfd2b01a4a11a452b753171ede02d140785821`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:73cfd2b`, production dry-run, production apply, canary/smoke, and Production Authenticated Dogfood E2E successfully. Evidence is compressed to run/job metadata and the harness coverage boundary; raw bodies, cookies, API keys, passwords, and request payloads are not stored.
- verified: GitHub Actions `Cloud Rollout` run `27823251419` completed with conclusion `success`; jobs `Production Dry Run`, `Production Apply`, and `Production Authenticated Dogfood E2E` completed with conclusion `success`.
- cannot claim: browser e2e, production real ordinary chat completion (`realChat=false`), quota exhaustion production evidence, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, or complete production-ready SaaS.

## 2026-06-19 - browser-e2e-evidence-foldback
- summary: Folded real local Chromium/CDP browser evidence into fixed release truth. The existing `tests/browser/research-main-path-runner.mjs` executed register, logout/login, API Key binding, ordinary `@科研` chat with mock upstream, `@论文` and `@基金` runtime gates, user-like input events, page-state evidence, and sanitized audit evidence. Raw cookies, API keys, passwords, page body, and request payloads are not stored.
- verified: `npm run verify:browser` passed with 2/2 tests, duration about 71s, path `research-main-path`, audit kinds `chat.completed` and `runtime_gate.required`, and upstream requests `>= 1`.
- cannot claim: production browser e2e, production real ordinary chat completion, quota exhaustion production evidence, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, or complete production-ready SaaS.

## 2026-06-19 - production-real-chat-dogfood-evidence-foldback
- summary: Folded Cloud Rollout run `27833052951` into fixed release truth. The run executed commit `116a56ce18e14c730be10628731d1a5fff9591c2`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:116a56c`, production dry-run, production apply, canary/smoke, and Production Authenticated Dogfood E2E successfully. User confirmed `OPL_PRODUCTION_DOGFOOD_REAL_CHAT=1`, so the dogfood harness executed one production ordinary chat completion, required assistant content, and required `chat.completed` audit evidence. Evidence is compressed to run/job metadata and harness coverage boundary; raw bodies, cookies, API keys, passwords, and request payloads are not stored.
- verified: GitHub Actions `Cloud Rollout` run `27833052951` completed with conclusion `success`; jobs `Production Dry Run`, `Production Apply`, and `Production Authenticated Dogfood E2E` completed with conclusion `success`; GitHub API reported head SHA `116a56ce18e14c730be10628731d1a5fff9591c2` and `updated_at` `2026-06-19T14:58:46Z`.
- cannot claim: production browser e2e, quota exhaustion production evidence, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, or complete production-ready SaaS.

## 2026-06-20 - production-b312-rollout-availability-dogfood-foldback
- summary: Folded Cloud Rollout run `27853332374` attempt 2 into fixed release truth. The run executed commit `b312f9c7af364c1ea2e9da5d57085279435f4a18`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:b312f9c`, production dry-run, production apply, canary/smoke, Production Availability Probe After Apply, and Production Authenticated Dogfood E2E successfully. Evidence is compressed to run/job metadata and harness coverage boundary; raw bodies, cookies, API keys, passwords, variable values, stdout bodies, and request payloads are not stored.
- verified: GitHub Actions `Cloud Rollout` run `27853332374` attempt 2 completed with conclusion `success`; jobs `Production Dry Run`, `Production Apply`, `Production Availability Probe After Apply`, and `Production Authenticated Dogfood E2E` completed with conclusion `success`; GitHub API reported head SHA `b312f9c7af364c1ea2e9da5d57085279435f4a18` and `updated_at` `2026-06-19T23:48:45Z`.
- cannot claim: production browser e2e, multi-node HA, complete production-ready SaaS, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, or production MedOPL readonly projection coverage because public GitHub job metadata does not expose `OPL_PRODUCTION_DOGFOOD_MEDOPL_READONLY` or dogfood stdout.

## 2026-06-20 - production-browser-e2e-success-foldback
- summary: Folded Cloud Rollout run `27863328297` into fixed release truth. The run executed commit `3725423dfa01ed67a2c2df9dd94863d920a972cf`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:3725423`, production dry-run, production apply, canary/smoke, Production Availability Probe After Apply, Production Authenticated Dogfood E2E, and Production Browser E2E successfully. Evidence is compressed to run/job metadata and harness coverage boundary; raw bodies, cookies, API keys, passwords, email values, variable values, stdout bodies, and request payloads are not stored.
- verified: GitHub Actions `Cloud Rollout` run `27863328297` completed with conclusion `success`; jobs `Production Dry Run`, `Production Apply`, `Production Availability Probe After Apply`, `Production Authenticated Dogfood E2E`, and `Production Browser E2E` completed with conclusion `success`; GitHub API reported head SHA `3725423dfa01ed67a2c2df9dd94863d920a972cf` and `updated_at` `2026-06-20T06:49:11Z`.
- cannot claim: multi-node HA, complete production-ready SaaS, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, or production MedOPL readonly projection coverage because public GitHub job metadata does not expose `OPL_PRODUCTION_DOGFOOD_MEDOPL_READONLY` or dogfood stdout.

## 2026-06-20 - production-3e69e3d-rollout-dogfood-browser-foldback
- summary: Folded Cloud Rollout run `27866718228` into fixed release truth. The run executed commit `3e69e3d38ed60b9aea96bd7d9c76ad65fe481135`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:3e69e3d`, production dry-run, production apply, canary/smoke, Production Availability Probe After Apply, Production Authenticated Dogfood E2E, and Production Browser E2E successfully. Evidence is compressed to run/job metadata and harness coverage boundary; raw bodies, cookies, API keys, passwords, email values, variable values, stdout bodies, and request payloads are not stored.
- verified: GitHub Actions `Cloud Rollout` run `27866718228` completed with conclusion `success`; jobs `Production Dry Run`, `Production Apply`, `Production Availability Probe After Apply`, `Production Authenticated Dogfood E2E`, and `Production Browser E2E` completed with conclusion `success`; GitHub API reported head SHA `3e69e3d38ed60b9aea96bd7d9c76ad65fe481135` and `updated_at` `2026-06-20T09:18:10Z`.
- cannot claim: multi-node HA, complete production-ready SaaS, long-term canary monitoring, alerting, error budget enforcement, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, team invite/RBAC/payment lifecycle, or production MedOPL readonly projection coverage because public GitHub job metadata does not expose `OPL_PRODUCTION_DOGFOOD_MEDOPL_READONLY` or dogfood stdout.

## 2026-06-20 - production-a3f7c39-rollout-dogfood-browser-foldback
- summary: Folded Cloud Rollout run `27876229568` into fixed release truth. The run executed commit `a3f7c399872a70332bd9e9465c05d11c9c2bd4ad`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:a3f7c39`, production dry-run, production apply, canary/smoke, Production Availability Probe After Apply, Production Authenticated Dogfood E2E, and Production Browser E2E successfully. Evidence is compressed to run/job metadata and harness coverage boundary; raw bodies, cookies, API keys, passwords, email values, variable values, stdout bodies, and request payloads are not stored.
- verified: GitHub Actions `Cloud Rollout` run `27876229568` completed with conclusion `success`; jobs `Production Dry Run`, `Production Apply`, `Production Availability Probe After Apply`, `Production Authenticated Dogfood E2E`, and `Production Browser E2E` completed with conclusion `success`; GitHub API reported head SHA `a3f7c399872a70332bd9e9465c05d11c9c2bd4ad` and `updated_at` `2026-06-20T15:57:07Z`.
- cannot claim: multi-node HA, complete production-ready SaaS, dashboard, alerting, error budget enforcement, production rollback record, automatic rollback, MedOPL runtime execution, billing/storage/node pool mutation, OPL execution/mutation, team invite/RBAC/payment lifecycle, or production MedOPL readonly projection coverage because public GitHub job metadata does not expose `OPL_PRODUCTION_DOGFOOD_MEDOPL_READONLY` or dogfood stdout.

## 2026-06-20 - production-canary-first-success-foldback
- summary: Folded Production Canary run `27874732529` into observability readiness truth. The scheduled workflow executed `Scheduled Production Availability Probe` against `https://opl.medopl.cn` with no kubeconfig, dogfood secrets, database secrets, MedOPL token, image credentials, or rollout mutation. Evidence is compressed to run/job metadata and the no-secret availability probe boundary.
- verified: GitHub Actions `Production Canary` run `27874732529` completed with conclusion `success`; job `Scheduled Production Availability Probe` completed with conclusion `success`; GitHub API reported head SHA `f3c7d10bebf6779b3bcfbaa6b1bd94fdd4e5cf6d` and `updated_at` `2026-06-20T14:56:48Z`.
- cannot claim: dashboard, alerting, error budget enforcement, automatic rollback, production rollback record, multi-node HA, complete production-ready SaaS, MedOPL runtime execution, billing/storage/node pool mutation, or OPL execution/mutation.
