...
| Production dogfood is secret-gated HTTP evidence | Production authenticated dogfood proves the production account/session, API Key binding, optional ordinary chat, runtime gate, and sanitized audit HTTP path. It is not browser e2e and not MedOPL runtime execution evidence. | `contracts/web-release-profile.json`, `.github/workflows/cloud-rollout.yml`, `scripts/cloud-rollout.mjs`, `deploy/web-cloud/RUNBOOK.md` |
