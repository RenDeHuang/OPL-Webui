...
| Verification uses fixed main lanes and dynamic targeted lanes | `current` is the daily main lane: `smoke + contract + health + go`. Browser, deploy, regression, and full lanes are explicit and selected by changed surface or release risk. Review gate includes changed-file lane advisory instead of a separate direct Go test step. | `AGENTS.md`, `scripts/test-classification.mjs`, `scripts/verify.mjs`, `scripts/lane-advisory.mjs`, `scripts/workflow-gate.mjs`, `scripts/repo-bloat-audit.mjs` |
