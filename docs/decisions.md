...
| Maintain static HTML/CSS/ESM web shell until evidence says otherwise | Current Web UI stays as static HTML, CSS, and browser ES modules served by the Go control plane. React/Vite/TypeScript migration trigger is browser e2e becoming a release gate plus repeated product-state complexity, component reuse pressure, typed client need, or Docker/CI build evidence showing the static shell is now limiting delivery. | `apps/web/`, `Dockerfile.cloud`, `tests/health/workflow-entrypoint.test.mjs` |
