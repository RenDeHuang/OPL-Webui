...
- Production HA readiness manifest is ready but not cloud-executed. `deploy/web-cloud/opl-webui.k8s.json` now declares `replicas=2`, `PDB minAvailable=1`, topology spread constraints, soft pod anti-affinity, and rolling update `maxUnavailable=0`. `contracts/web-release-profile.json` still requires two Ready Pods, distinct nodes, at least two healthy Ingress/CLB backends, canary/smoke, and production availability probe before any multi-node HA claim. Until cloud execution evidence is folded back, this repo cannot claim multi-node HA, CLB two-backend health, or zero-downtime rollout evidence.
