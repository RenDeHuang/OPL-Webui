...
- The five active non-HA gap steps now have repo-local phase acceptance boundaries: UI/UX visual baseline, MedOPL readonly foldback, runtime execution admission, commercial consumer contract, and operations evidence contracts. The runner can open the next step only when the current step is `done`; MedOPL readonly remains blocked by production evidence, runtime/commercial/operations remain gated by owner receipt or concrete consumer contracts.
