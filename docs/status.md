...
- `contracts/web-gap-phase-registry.json` turns broad goals into phase queues with owner surfaces, current-step objective, acceptance gates, owner receipt policy, next-step openers, required evals, evidence sources, typed blockers, and cannot-claim boundaries. `npm run gap:phase` reports `done` / `partial` / `blocked` / `not_started` status, `readyToAdvance`, and can write temporary summaries under `.runtime/phase-runs`.
...
- Production authenticated dogfood HTTP evidence executed successfully in GitHub Actions run `27876229568` for commit `a3f7c399872a70332bd9e9465c05d11c9c2bd4ad`, image `uswccr.ccs.tencentyun.com/webopl/opl-webui:a3f7c39`, after dry-run, production apply, canary/smoke, and availability probe success. production real ordinary chat completion and `chat.completed` audit evidence are confirmed by the production browser e2e lane in the same run. This is not MedOPL runtime execution evidence.
...
- Production browser e2e executed successfully in GitHub Actions run `27876229568` through the secret-gated real Chromium/CDP lane `node tests/browser/research-main-path-runner.mjs --production` and `Cloud Rollout` input `production_browser_e2e=true`. It ran after production apply and used the production dogfood account to verify real browser login, API Key binding, ordinary research chat, `@论文`/`@基金` runtime gates, and sanitized audit evidence against `https://opl.medopl.cn`.
...
- UI/UX product depth now has source-level Figma MCP evidence pinned in `contracts/web-gui-product-contract.json`: file `E8nYfNFc2D9P01FYZ8UwBW`, node `0:1`, required source files `src/app/App.tsx`, `src/styles/theme.css`, and `src/styles/index.css`. The current repo-local UI variant is `ai_native_research_home_v1`: the shell uses side navigation, a first-view center composer, starter chips, route-scoped Skill/工作流/Projects surfaces, one structured `@科研` artifact card instead of duplicated raw upstream transcript, a stable desktop right inspector work panel, and lightweight mobile bottom sheets. The visual contract freezes OPL green as the primary accent, rejects default tech blue/purple primary palettes, requires hover/active/focus-visible/disabled/loading states, and requires a `prefers-reduced-motion` path. It rejects Drive/storage ownership, runtime truth ownership, founder-plan upsell, unlimited compute claims, dashboard/CRM primary app, generic office/content/code/video skill surfaces, floating desktop inspector overlays, heavy mobile sheets, raw structured-result transcripts, and retired shell structures. Real local Chromium/CDP browser evidence captures desktop, tablet, mobile, and compact screenshots into git-ignored `.runtime/browser-visual/` artifacts and asserts no horizontal overflow, no static text overflow, inspector within viewport, desktop inspector stable panel, mobile inspector bottom sheet within height budget, artifact-first research result density, active inspector panel visibility, touch target pass, named controls pass, keyboard focus visibility, keyboard path pass, API Key modal focus trap pass, contrast pass, and no hidden overlay input interception through `npm run verify:browser`. Completed repo-local UI phases and repo-local accessibility closeout are folded into the GUI contract and tests; the active gap registry now keeps only `production_ui_quality_claim`, blocked on owner receipt, current-HEAD production evidence, and sanitized foldback. Long-term truth for this claim is limited to `contracts/web-gui-product-contract.json`, `contracts/web-gap-phase-registry.json`, `contracts/web-product-profile.json`, registered tests, `docs/status.md`, and `docs/active/README.md`. Temporary artifacts are `.runtime/browser-visual/*`, `.runtime/phase-runs/*`, production raw logs, raw screenshots, and CI raw output; they must not be committed, and only sanitized summaries may fold back. Complete UI/UX design system, assistive technology conformance, production visual polish, and UI/UX v1 production acceptance remain unclaimed.
...
- The five active non-HA gap steps now have repo-local phase acceptance boundaries: UI/UX `production_ui_quality_claim`, MedOPL readonly foldback, runtime execution admission, commercial consumer contract, and operations evidence contracts. The runner can advance only when the current step is `done`; UI/UX is blocked on owner receipt and current-HEAD production evidence, MedOPL readonly remains blocked by production evidence, and runtime/commercial/operations remain gated by owner receipt or concrete consumer contracts.

## Cannot Claim

- 还不是完整 production-ready SaaS。
- 本阶段没有可公开确认的 production readonly projection dogfood；run `27876229568` 的 dogfood job 成功，但公开 GitHub metadata 不暴露 `OPL_PRODUCTION_DOGFOOD_MEDOPL_READONLY` 变量值或 dogfood stdout，因此不能 claim production readonly projection evidence。
- 本阶段没有证明 multi-node HA；当前 availability probe 只证明 public HTTP endpoints 的重复可用性。
- 本阶段没有 dashboard、alerting、error budget enforcement、production rollback record 或 automatic rollback。
...
- 不能 claim `ui_ux_v1_production_accepted`、complete UI/UX design system、assistive technology conformance 或 production visual polish；这些需要 human owner receipt、当前 HEAD 的生产浏览器/截图证据、assistive technology review 和 sanitized foldback。
...
2. Continue UI/UX depth through Figma MCP first. The repo-local responsive visual QA and accessibility closeout evidence is captured and the current phase is `production_ui_quality_claim`; do not claim production UI quality until a human owner receipt, current-HEAD production evidence, assistive technology review if claiming full accessibility, and sanitized foldback exist.
