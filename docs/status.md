...
- The five active non-HA gap steps now have repo-local phase acceptance boundaries: UI/UX `production_ui_quality_claim`, MedOPL readonly foldback, runtime execution admission, commercial consumer contract, and operations evidence contracts. The runner can advance only when the current step is `done`; UI/UX is blocked on owner receipt, production evidence, and accessibility closeout, MedOPL readonly remains blocked by production evidence, and runtime/commercial/operations remain gated by owner receipt or concrete consumer contracts.
...
- 不能 claim `ui_ux_v1_production_accepted`、complete UI/UX design system、assistive technology conformance 或 production visual polish；这些需要 human owner receipt、生产浏览器/截图证据、accessibility closeout boundary 和 sanitized foldback。
...
2. Continue UI/UX depth through Figma MCP first. The repo-local responsive visual QA evidence is captured and the current phase is `production_ui_quality_claim`; do not claim production UI quality until a human owner receipt, production evidence, accessibility closeout boundary, and sanitized foldback exist.
