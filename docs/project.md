...
The durable UI source truth is `contracts/web-product-profile.json#/uiSourceTruth`: Figma Make `UI_UX for Commercial Launch`, file key `1MNO5l7PQYKZVNqQgw6DGS`, source URL `https://www.figma.com/make/1MNO5l7PQYKZVNqQgw6DGS/UI-UX-for-Commercial-Launch?p=f&t=c6JjbjSRg22dfHKu-0`. UI implementation work must read `src/app/App.tsx`, `src/styles/theme.css`, `src/styles/globals.css`, and relevant components/images, then translate the source into the current OPL-Webui architecture. Figma generated dependencies and mock data are not product truth.
