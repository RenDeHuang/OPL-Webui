...
4. Production browser e2e harness is ready and has been attempted in GitHub Actions runs `27862151414` and `27862789365`; latest run `27862789365` failed at `Production Browser E2E`. It still uses `production_browser_e2e=true` in `Cloud Rollout` with `apply=true`, `OPL_PRODUCTION_BROWSER_E2E=1`, `OPL_DOGFOOD_EMAIL`, `OPL_DOGFOOD_PASSWORD`, and `OPL_DOGFOOD_API_KEY`; until the failing browser lane is fixed and rerun successfully, the repo cannot claim production browser e2e evidence.
