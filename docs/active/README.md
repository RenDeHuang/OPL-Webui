...
当前是 `saas-dogfood-guardrails`：OPL-Webui 已在 `figma-2-21-ui-alignment-production` 和 `1fc361d` Figma workbench UI production closeout 后，给 one-person-lab-web 主路径补齐最小 dogfood 护栏：普通 chat 有 Webui-side monthly quota/abuse guard，register/login/API Key/chat/runtime gate/upstream failure 写 sanitized audit event。`one-person-lab` 是 framework/runtime/contract truth；`one-person-lab-app` 是桌面端产品语义参考，包括 chat-first、complex knowledge work、research/grant/presentation foundry、progress/files/deliverables。
