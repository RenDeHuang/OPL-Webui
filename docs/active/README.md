...
16. UI/UX Product Depth now requires Figma MCP before further UI work. Current source evidence is pinned to Figma Make file `E8nYfNFc2D9P01FYZ8UwBW`, node `0:1`, with `src/app/App.tsx`, `src/styles/theme.css`, and `src/styles/index.css` as source files. The current repo-local UI variant is `clean_workbench_v1`: the shell uses a workbench command band, summary strip, research skill launcher, chat/task view, right inspector tabs, and running/blocked turn state while the old `hero-shell` markup is retired. Rejected patterns are Drive/storage ownership, runtime truth ownership, founder-plan upsell, unlimited compute, dashboard/CRM primary app, and generic office/content/code/video skills. The repo-local responsive visual QA is captured by real Chromium/CDP desktop, tablet, mobile, and compact screenshots under git-ignored `.runtime/browser-visual/`, plus browser assertions for overflow, static text fit, inspector visibility, touch targets, named controls, keyboard focus visibility, and hidden-overlay input safety. The current UI/UX phase is `production_ui_quality_claim`, but it is pending owner receipt, production evidence, accessibility closeout boundary, and sanitized foldback. Long-term truth is limited to `contracts/web-gui-product-contract.json`, `contracts/web-gap-phase-registry.json`, `contracts/web-product-profile.json`, registered tests, `docs/status.md`, and this baton. Temporary artifacts are `.runtime/browser-visual/*`, `.runtime/phase-runs/*`, production raw logs, raw screenshots, and CI raw output; they stay out of git and only sanitized summaries fold back. Complete UI/UX design system, assistive technology conformance, production visual polish, and owner receipt remain pending.
