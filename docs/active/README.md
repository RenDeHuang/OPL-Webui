...
7. Production observability baseline v1 is now wired into `/metricsz`, `scripts/cloud-rollout.mjs --availability-probe`, and `contracts/web-release-profile.json`. It produces no-secret endpoint sample summaries for release closeout; it is not HA, long-term canary, dashboard, alerting, or error-budget evidence.
8. Manual rollback evidence is now wired into `Cloud Rollout` through `rollback=true` and `node scripts/cloud-rollout.mjs --rollback`. It is production environment approved and reuses canary/smoke gates; it is not automatic rollback and has no production rollback run yet.
9. Commercial account lifecycle is now a readonly Web account projection: `GET /api/account/commercial-status` returns personal account, tenant role, active lifecycle state, and forbidden mutation flags. The browser settings surface renders lifecycle, quota, and audit summaries. It must not grow into team invite, RBAC, payment, billing owner, workspace-visible UI, storage, node pool, or runtime ownership without a new owner/consumer/test gate.
10. User-facing reliability status is now rendered in the browser shell for login/key/quota/runtime/upstream/service/network states. It is a sanitized UX view model only and must not include raw provider errors, API keys, DB URLs, private state, or artifact bodies.
11. Frontend engineering stays static HTML/CSS/ESM until browser evidence and product complexity justify React/Vite/TypeScript migration.
12. Retired Go task projection and OPL task-route surfaces must stay tombstoned; current DB canary validates the Web app schema.
