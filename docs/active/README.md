...
9. Production observability baseline v1 is now wired into `/metricsz`, `scripts/cloud-rollout.mjs --availability-probe`, `contracts/web-release-profile.json`, and the no-secret scheduled workflow `.github/workflows/production-canary.yml`; release evidence is folded back to run `28039468173`, and scheduled canary first success is folded back to run `27874732529`. Operations readiness now has P0/P1/P2 contracts in `contracts/web-release-profile.json`: P0 launch operations, P1 commercial operations, and P2 SLA/HA operations gates are explicit. This is partial readiness only; external evidence for rollback record, alert route, DB restore drill, dashboard URL, production load, HA topology, SLO enforcement, and automatic rollback remains pending.
...
19. Operations maturity now means baseline plus P0/P1/P2 readiness contracts: dashboard URL, alert route, error budget/SLO, DB restore drill, production load, production rollback execution, HA topology, and automatic rollback remain unadmitted until sanitized external evidence is folded back. Current can claim release observability baseline, scheduled canary first success, rollback-record contract, and P0/P1/P2 contract boundaries only.
