...
16. UI/UX Product Depth now requires Figma MCP before further UI work. Current source evidence is pinned to Figma Make file `E8nYfNFc2D9P01FYZ8UwBW`, node `0:1`, with `src/app/App.tsx`, `src/styles/theme.css`, and `src/styles/index.css` as source files. The current repo-local UI variant is `ai_native_research_home_v1`: the shell uses side navigation, a first-view center composer, starter chips, route-scoped Skill/工作流/Projects surfaces, one structured `@科研` artifact card instead of duplicated raw upstream transcript, a stable desktop right inspector work panel, and lightweight mobile bottom sheets. Rejected patterns are Drive/storage ownership, runtime truth ownership, founder-plan upsell, unlimited compute, dashboard/CRM primary app, generic office/content/code/video skills, default tech blue/purple primary palettes, floating desktop inspector overlays, heavy mobile sheets, raw structured-result transcripts, and retired shell structures. Repo-local responsive visual QA, OPL green, full interaction states, `prefers-reduced-motion`, keyboard path, API Key modal focus trap, contrast closeout, artifact-first result density, stable desktop inspector, and mobile sheet height budget are folded into `contracts/web-gui-product-contract.json` and registered tests; they are no longer active phase contracts. The current UI/UX phase is only `production_ui_quality_claim`, blocked on owner receipt, current-HEAD production evidence, and sanitized foldback. Long-term truth is limited to `contracts/web-gui-product-contract.json`, `contracts/web-gap-phase-registry.json`, `contracts/web-product-profile.json`, registered tests, `docs/status.md`, and this baton. Temporary artifacts are `.runtime/browser-visual/*`, `.runtime/phase-runs/*`, production raw logs, raw screenshots, and CI raw output; they stay out of git and only sanitized summaries fold back. Complete UI/UX design system, assistive technology conformance, production visual polish, and owner receipt remain pending.
17. Runtime execution remains `not_admitted`: no OPL install, repair, module exec, mutation, artifact body, private state, or domain verdict until structured admission conditions are admitted for Go-side runtime execution contract, registered allowlist eval, command allowlist, human authorization boundary, tenant audit, and artifact/body authority contract. `webRoutesMayMutateRuntime=false` is the current contract.
18. Commercial lifecycle remains readonly personal projection. Any team/invite/RBAC/pricing/subscription/payment expansion is blocked by structured expansion conditions for real consumer, surface contract, registered tests, MedOPL billing authority preservation, and payment processor contract. `webRoutesMayMutateBilling=false` is the current contract.
...
20. Current non-HA gap phases are now explicit step gates rather than a broad wish list: UI/UX is in `production_ui_quality_claim` and stays blocked until owner receipt, current-HEAD production evidence, and sanitized foldback exist; MedOPL readonly foldback is blocked on secret-gated production evidence; runtime execution is blocked on human owner receipt plus allowlist/admission evidence; commercial depth is blocked on buyer/operator workflow plus expansion-condition evidence; operations maturity is blocked on operations-owner selection plus rollback-record production evidence.
