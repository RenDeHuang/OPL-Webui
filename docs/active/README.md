...
9. Production observability baseline v1 is now wired into `/metricsz`, `scripts/cloud-rollout.mjs --availability-probe`, `contracts/web-release-profile.json`, and the no-secret scheduled workflow `.github/workflows/production-canary.yml`; release evidence is folded back to run `28039468173`, and scheduled canary first success is folded back to run `27874732529`. Operations readiness now has P0/P1/P2 contracts in `contracts/web-release-profile.json`: P0 launch operations, P1 commercial operations, and P2 SLA/HA operations gates are explicit. `productionLaunchCloseout` is the final release decision receipt contract and only accepts sanitized explicit evidence for soak, load, rollback, canary, alerting, DB restore, monitoring, HA, and SLO through `npm run release:evidence -- --launch-closeout-json <sanitized-json>`. This is partial readiness only; external evidence for final release receipt, rollback record, alert route, DB restore drill, dashboard URL, production load, long soak, HA topology, SLO enforcement, and automatic rollback remains pending.
...
19. Operations maturity now means baseline plus P0/P1/P2 readiness contracts plus final release decision receipt contract: dashboard URL, alert route, error budget/SLO, DB restore drill, production load, long soak, production rollback execution, HA topology, final release approval, and automatic rollback remain unadmitted until sanitized external evidence is folded back. Current can claim release observability baseline, scheduled canary first success, rollback-record contract, P0/P1/P2 contract boundaries, and final receipt contract only.
