...
- `runtime.deploy.cloud-mvp-shape`: `deploy/cloud-mvp/opl-webui.k8s.json` 固定 `opl.medopl.cn`、namespace、`uswccr` cloud stable HTTP image、imagePullSecret、nodeSelector、resources、`qcloud` ingress class、NodePort `32258`、`4173`、`/healthz`、`/readyz`、`cloud_mvp` env 和 `OPL_DATABASE_URL` secretKeyRef。
- `runtime.deploy.cloud-mvp-runbook`: `deploy/cloud-mvp/RUNBOOK.md` 提供云端/VPC runner handoff 步骤，覆盖 TCR/CCR build/push、Secret 创建、镜像替换、apply、canary、HTTP smoke、DNS、504 排障和 rollback；真实 kubeconfig、数据库密码、云 API key 只能由外部路径或执行环境注入。
...
- 当前 runtime 不包含 HTTPS production hardening、队列、计费、object storage、worker、完整 production ready 运行证据或真实 OPL mutation。
