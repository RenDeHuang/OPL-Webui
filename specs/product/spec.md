...
- `product.production-dogfood-e2e`: production authenticated dogfood harness 必须默认跳过，并由 GitHub `production` Environment secrets 或人工本地输入提供专用 test account/API Key；必须覆盖 register/login fallback、current session、API Key binding、raw key 不回显、fixed gateway、可选普通 chat completion、`@基金` MedOPL Runtime gate 和 audit events；不能打印 raw API Key、password、request body、完整 response body 或 `opl_session`；不能注入 `KUBECONFIG`、DB secret、MedOPL token 或 TCR 凭据；不能执行 kubectl、连接 PostgreSQL、连接 MedOPL production、伪造 runtime 或执行真实 OPL。
...
- 当前已固化 production authenticated dogfood e2e plan/harness，但还不能宣称已在线执行；真实普通 chat completion 需要专用测试 API Key 和 `OPL_PRODUCTION_DOGFOOD_REAL_CHAT=1`。
