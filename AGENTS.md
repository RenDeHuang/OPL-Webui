...
- 每次正式开发开始前必须根据改动面决定 targeted lane：`frontend/web/**` 跑 `verify:smoke` 与 `verify:browser`；`contracts/web-page-state-matrix.json` 跑 `verify:contract` 与 `verify:browser`；`backend/control-plane-go/**` 跑 `verify:contract` 与 `verify:go`；`deploy/**`、`.github/workflows/**`、`scripts/cloud-rollout.mjs` 跑 `verify:deploy` 与 `verify:health`；OPL bridge/runtime gate 变更跑 `verify:contract`、`verify:go` 和 `verify:full`。
