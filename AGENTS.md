...
- 默认主验证入口是 `npm run verify`，即 daily `fast = fast + ui + api + health-light + go-light`；review gate 是 `npm run gate:review`。
...
- 测试采用 fast 日常 lane 加动态 targeted lane：`fast` 守普通开发快检，`ui` 守 UI source/view-model 边界，`api` 守 Go HTTP/API，`smoke` 守基础入口，`contract` 守 API/page-state/BYOK/tenant/OPL 边界，`health` 守治理和清退，`go` 守 Go control plane，`browser:golden` 守浏览器主路径，完整 `browser`、`integration`、`release/deploy`、`regression`、`real-medopl` 和 `full` 用于发布、高风险或显式验证。
...
- 每次正式开发开始前必须根据改动面决定 targeted lane：`frontend/web/**` 跑 `verify:ui`、`verify:smoke`、`verify:interaction` 与 `verify:browser:golden`；`contracts/web-page-state-matrix.json` 跑 `verify:interaction`、`verify:contract` 与 `verify:browser:golden`；`backend/control-plane-go/**` 跑 `verify:api` 与 `verify:go`；`deploy/**`、`.github/workflows/**`、`scripts/cloud-rollout.mjs` 跑 `verify:deploy` 与 `verify:health`；OPL bridge/runtime gate 变更跑 `verify:integration`，发布风险变更再跑 `verify:full`。
