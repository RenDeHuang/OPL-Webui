...
test('unauthenticated public task entry opens login and restores selected task after auth', { timeout: 180000 }, () => {
  ...
  assert.equal(evidence.pageStates.authSurfaceSlice, 'figma_auth_surface_slice');
  assert.equal(evidence.pageStates.authSurface.centeredCard, true);
  assert.equal(evidence.pageStates.authSurface.cardVisible, true);
  assert.equal(evidence.pageStates.authSurface.tabCount, 2);
  assert.equal(evidence.pageStates.authSurface.selectedTab, 'login');
  assert.equal(evidence.pageStates.authSurface.visibleSubmitCount, 1);
  assert.equal(evidence.pageStates.authSurface.visibleSubmitAction, 'login');
  assert.equal(evidence.pageStates.authSurface.brandText, 'One Person Lab');
  assert.equal(evidence.pageStates.authSurface.subtitleText, 'AI 原生科研工作台');
  assert.equal(evidence.pageStates.authSurface.emailPlaceholder, 'researcher@lab.edu');
  assert.equal(evidence.pageStates.authSurface.passwordPlaceholder, '********');
  assert.equal(evidence.pageStates.authSurface.hasPasswordToggle, true);
  assert.equal(evidence.pageStates.authSurface.hasTermsCopy, true);
