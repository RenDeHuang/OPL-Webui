...
export async function startMockChatUpstream() {
  const requests = [];
...
export async function startControlPlane(upstreamBaseUrl, cleanup, repoRoot) {
...
export async function startBrowser(binary, cleanup) {
  ...
  cleanup.push(() => closeChildProcess(child, () => rmSync(userDataDir, { recursive: true, force: true })));
...
export async function createPageTarget(devtoolsBaseUrl) {
...
export async function userClick(cdp, selector) {
...
export async function activate(cdp, selector) {
...
export async function focusElement(cdp, selector, options = {}) {
  await cdp.send('Runtime.evaluate', {
...
export async function waitForAuthState(cdp, authState, label) {
  await waitFor(
    cdp,
...
export async function waitForBoundOrUnboundAuthState(cdp, label) {
  await waitFor(
    cdp,
...
export async function waitForAuthStateOrMessage(cdp, authState, messagePattern, label) {
...
export async function describePageState(cdp, message) {
  const state = await evaluateJSON(cdp, `({
...
export async function describeResearchResultState(cdp, message) {
  const state = await evaluateJSON(cdp, `({
    message: ${JSON.stringify(message)},
    authState: document.body.dataset.authState,
    chatState: document.body.dataset.chatState,
...
export async function keyPress(cdp, key) {
  await cdp.send('Input.dispatchKeyEvent', { type: 'keyDown', ...key });
  await cdp.send('Input.dispatchKeyEvent', { type: 'keyUp', ...key });
}

export async function waitFor(cdp, expression, describeFailure, timeoutMs = 30000) {
...
export async function assertPage(cdp, expression, label) {
...
export async function captureScreenshot(cdp, relativePath, repoRoot) {
...
export async function waitUntil(check, timeoutMs = 30000, describeFailure) {
...
export function normalizeBaseUrl(value) { return String(value || '').replace(/\/+$/, ''); }
export function sanitizeBaseUrl(value) { const url = new URL(value); return `${url.protocol}//${url.host}`; }
