...
test('active baton and tombstone preserve next-agent context without becoming machine truth', () => {
  ...
  assert.match(active, /28142197152/);
  assert.match(active, /0728dc448d4b8105494e6e6b652ad8a43e1e19db/);
...
test('product contracts keep OPL-WebUI as one-person-lab-web instead of standalone SaaS backend', () => {
  ...
  assert.equal(release.productionObservabilityBaseline.latestSuccessfulRun.runId, 28142197152);
  ...
  assert.match(runbook, /28142197152/);
  assert.match(runbook, /0728dc4/);
