...
test('one-person-lab-web contracts define product truth instead of prose specs', () => {
  ...
  assert.deepEqual(pageStates.shellStates.sideNavigation.actions.map(({ id, label, kind, targetView, localOnly, consumer, forbiddenApis }) => [id, label, kind, targetView, localOnly, consumer, forbiddenApis.join('|')]), [['home', '新聊天', 'route', 'home', true, 'composer_focus', ''], ['search', '搜索聊天', 'sheet', 'home', true, 'project_window_search', '/api/search'], ['files', '文件库', 'local_handoff', 'home', true, 'file_material_handoff_to_medopl', ''], ['workflows', '已安排/任务', 'route', 'workflows', true, 'scheduled_tasks_and_workflow_library', ''], ['skills', '应用 / Skills', 'route', 'skills', true, 'skill_library', ''], ['more', '更多', 'route', 'more', true, 'empty_overflow_menu', '']]);
  assert.deepEqual(pageStates.shellStates.librarySurfaces, { skills: { scopes: ['my_skills', 'opl_skills'], actions: ['import_skill'] }, workflows: { scopes: ['my_workflows', 'opl_workflows'], actions: ['create_workflow', 'import_workflow'] }, projects: { scopes: ['my_project_windows'], actions: ['continue_window'], requiredApis: ['/api/tasks', '/api/tasks/{taskId}'], projection: 'project_window_refs_status_metadata_only', doesNotProve: ['dedicated project/window persistence API'] } });
  ...
  assert.deepEqual(gui.informationArchitecture.sideNavigationTaskOwnership, { home: 'compose_or_start_research_task', search: 'open_project_window_search', files: 'file_material_handoff_to_medopl', workflows: 'browse_scheduled_tasks_and_workflows', skills: 'browse_or_import_skill_entries', more: 'route_empty_overflow_surface' });
  assert.deepEqual(gui.informationArchitecture.sideNavigationActionMap, { home: { action: 'route_home_and_focus_composer', targetView: 'home', ownerSurface: 'browser_shell', consumer: 'composer_focus' }, search: { action: 'open_search_sheet', targetView: 'home', ownerSurface: 'browser_shell', consumer: 'project_window_search' }, files: { action: 'open_file_attach_handoff', targetView: 'home', ownerSurface: 'browser_shell', consumer: 'file_material_handoff_to_medopl' }, workflows: { action: 'route_workflows_page', targetView: 'workflows', ownerSurface: 'browser_shell', consumer: 'scheduled_tasks_and_workflow_library' }, skills: { action: 'route_skills_page', targetView: 'skills', ownerSurface: 'browser_shell', consumer: 'skill_library' }, more: { action: 'route_more_overflow_surface', targetView: 'more', ownerSurface: 'browser_shell', consumer: 'empty_overflow_menu' } });
  ...
  assertIncludesAll(gui.designQualityGates.navigation.required, ['search_is_project_window_search', 'more_is_empty_overflow', 'bottom_avatar_account_entry', 'optional_model_selector'], 'navigation design quality gate');
  ...
  assert.deepEqual(gui.components.find((component) => component.id === 'ProjectWindowCenter').actions, ['continue_window']);
