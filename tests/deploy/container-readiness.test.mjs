...
test('container deploy entrypoint builds the Go control plane only', () => {
  ...
  assert.match(dockerfile, /go build .*\.\/backend\/control-plane-go\/cmd\/opl-webui-control-plane/);
...
test('cloud image recipe materializes OPL CLI into the runtime path', () => {
  ...
  for (const required of ['*', '!Dockerfile.cloud', '!go.work', '!backend/control-plane-go/**', '!frontend/web/**']) {
...
test('docker build context excludes runtime bloat', () => {
  ...
  for (const required of ['*', '!.dockerignore', '!Dockerfile', '!go.work', '!services', '!backend/control-plane-go', '!backend/control-plane-go/**', '!apps', '!frontend/web', '!frontend/web/**']) {
