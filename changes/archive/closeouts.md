...
- summary: 采用 Figma node `2:21` 作为 UI alignment delta，将首页推进为“严肃工作的 AI 工作台”：大输入框、能力 capsules、五件事结构、Foundry 启动中心、账号与凭据状态区、冷灰白卡和蓝色编号；保留 fixed base_url、auth/API Key binding 和 @OPL MedOPL Runtime gate，不恢复 `demoData.mjs`，不调用 `/api/mvp/task`。verified: RED/GREEN web smoke+contract；browser desktop/mobile/#settings/@基金 gate；final gates recorded in commit evidence。cannot claim: production rollout、真实 OPL execution/mutation、object storage、billing、node pool 生命周期。
