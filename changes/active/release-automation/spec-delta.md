...
- 新增 `.github/workflows/release-image.yml`，作为 Phase 2 build/push 和 staging rollout 入口。
- 新增 `.github/workflows/cloud-rollout.yml`，作为 Phase 3/4 self-hosted runner rollout 入口。
...
- 更新 `tests/contract/cloud-rollout-helper.test.mjs`，验证 `OPL_BASE_URL` 和 staging namespace dry-run。
