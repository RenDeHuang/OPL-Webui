...
Test truth is lane-based, not case-count-based. `scripts/test-classification.mjs` owns test taxonomy, lane membership, cost, lifecycle role, risk triggers, and verify-suite metadata. `npm run verify` runs the daily `fast` suite only; `npm run verify:dev` selects impacted lanes from the current diff for ordinary product engineering. Dynamic lane checks use changed files plus fresh verification evidence; `npm run lane:advisory -- <files...>` only explains the targeted lanes to run. Browser golden, full browser, integration, release/deploy, real-MedOPL, and full verification are explicit lanes for changed surfaces and release risk, not hidden inside every UI edit.
...
Regression tests are temporary guards, not permanent inventory. The registry creates a `regression` lane only when an active `regression-guard` entry exists. Any such entry must carry retirement metadata, and when its condition is satisfied the same cleanup removes the test file, registry entry, and any needed no-resurrection tombstone.
